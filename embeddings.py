...
class Embedding:
    ...
    def question_ranked_by_relatedness(self,
        question: str,
        embedding_info: list,
        top_n):
        ...
        for i in range(len(embedding_info)):
            score = 1 - spatial.distance.cosine(question_embedding, embedding_info[i][1])
    ...
    def get_matched_embeddings(self, questions, embedding_list, top_n=5): 
        ...
        for i in range(len(questions)):
            relatednesses = self.question_ranked_by_relatedness(questions[i], embedding_list, top_n=top_n)
            matched_output+=f"Question {i+1}:\n{questions[i]}:\n{str(relatednesses)}\n\n"
